...
do_mount_root()
{
   ...
   # 💡 【修正点】即エラーにせず、デバイスが認識されるまでウェイトをかけるループ
   # rootdelayが未指定ならデフォルトで最大10秒待ちます
   MAX_WAIT=${rootdelay:-10}
   COUNTER=0
   
   echo "Waiting for root device $device (Timeout: ${MAX_WAIT}s)..."
   while [ ! -b "$device" ] ; do
       sleep 1
       COUNTER=$((COUNTER + 1))
       if [ $COUNTER -ge $MAX_WAIT ]; then
           # タイムアウト時間を過ぎても見つからない場合のみエラー画面へ
           no_device $device
           problem
           break
       fi
